...
def get_grid(image, chip_size):
    ...
    for x_idx, x_coord in enumerate(range(0, width - pixels_ignored_in_x, chip_size)):
        for y_idx, y_coord in enumerate(
            range(0, height - pixels_ignored_in_y, chip_size)
        ):
...
def save_stats_plot(
    file, save, color_dict, title, show_count=True, show_y_axis=False, bold_edges=False
):
    # read data
    if isinstance(file, str):
        ...
        print(
            "invalid file type provided to save_stats_plot. Required: either filepath or data frame"
        )
        return
    ...
    if bold_edges:
        ...
        edge = rgba_colors
    plt.bar(
        categories, counts, color=rgba_colors, width=0.8, edgecolor=edge, linewidth=5
    )
...
if __name__ == "__main__":
    ...
    num_cols = (
        40  # NOTE: this should match whatever was used to chip the original image
    )
    colors = {
        "vacuum": (0, 0, 255),
        "particles": (255, 0, 0),
    ...
    plt.rcParams.update(
        {"font.family": "Arial", "font.size": 20, "font.weight": "light"}
    )
    save_stats_plot(
        csv_path,
        stats_save_path,
